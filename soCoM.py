...
UN = 10 #Numbers of user equipment
...
class MEC(object):
    ...
    def writelog(self,env,fn, name, value, timeslot = 5000):
        ...
        # f = open('.\\dataDueling\\USER_'+str(fn)+'_'+str(name)+'_'+str(value)+'.data','w')
        f = open('.\\data' + MODEL_USED + '\\USER_'+str(fn)+'_'+str(name)+'_'+str(value)+'.data','w')
        ...
        while True:
            ...
            sumreward = self.REWARD
            f = open('.\\data' + MODEL_USED + '\\USER_'+str(fn)+'_'+str(name)+'_'+str(value)+'.data','a')
            oneline = str(env.now/1000)+'\t'+str(throu)+'\t'+str(age)+'\t'+str(run)+'\t'+str(comm)+'\t'+str(energy)+'\t'+str(sumreward)+'\n'
            f.write(oneline)
        f.close()
    
    def writeoffload(self,fn, name, value):
        f = open('.\\data' + MODEL_USED + '\\JOB_'+str(fn)+'_'+str(name)+'_'+str(value)+'.data','w')
    ...
    def refreshsys(self,env,WAITING_LEN,name='',value='',flag = 0):
        if flag ==1:
            f = open('.\\data' + MODEL_USED + '\\ACTION_'+str(name)+'_'+str(value)+'.data','w')
        ...
        while True:
            ...
            if flag ==1:
                f = open('.\\data' + MODEL_USED + '\\ACTION_'+str(name)+'_'+str(value)+'.data','a')
